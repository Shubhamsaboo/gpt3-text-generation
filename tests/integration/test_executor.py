...
def test_complete():
    ...
    flow = Flow().add(uses=Gpt3, uses_with=({'api_key': _API_KEY}))
...
def test_complete_wo_max_tokens():
    ...
    flow = Flow().add(uses=Gpt3, uses_with={'api_key': _API_KEY, 'max_tokens': _max_tokens})
...
def test_complete_wo_temperature():
    ...
    flow = Flow().add(uses=Gpt3, uses_with={'api_key': _API_KEY, 'temperature': _temperature})
